...
- 49,837 个唯一标签均已写入完整位置表，“其他标签”当前为 0。运行时优先采用人工复核与稳定规则，只有仍未锁定的长尾标签才使用名称、中文别名和 Wiki 定义的全分类语义候选。
- 版本升级会迁移旧分类位置；已隐藏的内置标签按标签名继续保持隐藏，自建标签与分类设置不会因内置标签重排而丢失。
- 每个唯一标签恰好归档一次；低可信长尾标签持续进入 Wiki 复核队列，不再允许语义相似度覆盖已确认的分类。
...
`wiki_pairwise_taxonomy_audit.py` 是仅供实验的离线审计工具，不是应用运行依赖，也不作为人工分类结论。它读取每个标签的英文名、中文名、别名和完整 Wiki 定义，与 357 个语义细分类及 52 个父级大分类交叉编码比较；实验候选、分数和断点写入被忽略的 `audit_output` 与 `.audit-cache` 目录，绝不直接覆盖人工确认项。首次运行需单独安装审计依赖：
