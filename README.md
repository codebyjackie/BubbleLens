...
`wiki_pairwise_taxonomy_audit.py` 是仅供实验的离线审计工具，不是应用运行依赖，也不作为人工分类结论。它读取每个标签的英文名、中文名、别名和完整 Wiki 定义，与 363 个语义细分类及 52 个父级大分类交叉编码比较；实验候选、分数和断点写入被忽略的 `audit_output` 与 `.audit-cache` 目录，绝不直接覆盖人工确认项。首次运行需单独安装审计依赖：
